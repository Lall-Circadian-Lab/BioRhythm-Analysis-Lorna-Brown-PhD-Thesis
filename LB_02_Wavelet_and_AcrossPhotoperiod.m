...
%
% Wavelet settings
%   Morlet CWT ('amor'), period limits 60–1680 min, jet scalograms,
...
% Analysis defaults for wavelet and across-photoperiod steps.
    ...
    cfg.periodLimitsMinutes = [60, 1680]; % 1–28 h (Core match)
    cfg.scalogramYTicksHours = 0:4:28;
    cfg.topNPeaks = 5;
...
% Photoperiods are ordered by light duration from the summary MAT files.
    ...
    lb_plot_period_scatter(allPeriod, fullfile(figFolder, ['Period_comparison_RAW.' theme.format]), ...
        'Period comparison RAW (exploratory): rank-1 peak', 'Dominant period (h) — rank 1', [0 28]);
    ...
    lb_plot_period_distributions(allPeriod, figFolder, 'Period_distribution_RAW', theme, ...
        'RAW rank-1 period distribution', 'Dominant period (h) — rank 1', [0 28]);
